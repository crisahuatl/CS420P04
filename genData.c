...
void fileGen(char* file) {
    ...
    //fill buffer with a repeating sequence of the alphabet a-z
    //using the 'a' as a starting point, other letters in the alphabet can be used by adding integers to generate the ASCII code
    //Ex: 'a' + 1 = 'b'   'b' + 1 = 'c' etc...
    //The modulous operation restricts this process to the 26 characters of the alphabet
    ...
    while (bytesWritten < FILE_SIZE) {
        ...
        int result = fwrite(buffer, 1, chunkSize, fptr); //writes one chunk at a time to file. Function returns the total number of successfully written elements
    ...
    fclose(fptr);
    printf("File written successfully, Total Bytes Written: %d\n", bytesWritten);
